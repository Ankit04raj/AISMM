    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {platformList.map((pKey) => {
          ...
          const linkedAccount = selectAccountForPlatform(userAccounts, pKey);
          const status = normalizeConnectionStatus(linkedAccount?.connection_status);
          const isLiveOAuth = status === 'connected_live';
          const isTokenExpired = status === 'token_expired';
          const isConnected = isLiveOAuth || isTokenExpired;
            ...
            >
              <div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-[11px] font-bold px-2.5 py-0.5 rounded-full flex items-center gap-1.5 font-mono ${
                    ...
                    {isTokenExpired && <AlertTriangle className="w-3 h-3 text-amber-400" />}
                    <span>{labelForConnectionStatus(status)}</span>
                  </span>
                </div>
              ...
              <div className="pt-4 border-t border-[#1E293B] flex items-center justify-between gap-2">
                <span className="text-[10px] text-slate-500 font-mono">
                  {isLiveOAuth ? "OAuth 2.0 Live Active" : "OAuth Handshake Required"}
                </span>
                <div className="flex items-center gap-2">
                  {linkedAccount && shouldOfferReconnect(status) && (
                    <button
                    ...
                    >
                      <Plus className="w-3.5 h-3.5" />
                      <span>{connectingPlatform === pKey ? "Redirecting..." : "Reconnect via OAuth"}</span>
                    </button>
                  )}
                  {isConnected ? (
                    <>
