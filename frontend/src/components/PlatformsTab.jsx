...
export default function PlatformsTab() {
  ...
  const handleConnect = async (pKey) => {
    ...
    try {
      ...
      if (oauth?.authorization_url) window.location.assign(oauth.authorization_url);
    } catch (err) {
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Manage authenticated platform adapters, OAuth tokens, and dynamic capability contracts
          </p>
        </div>
