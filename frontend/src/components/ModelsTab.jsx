...
export default function ModelsTab() {
  ...
  return <div className="space-y-6 animate-fadeIn"><div className="notice">Experimental decision support. Current ML models use synthetic training/evaluation data; scores are not validated predictions of your real audience. Review recommendations before acting.</div>
    <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
      <div><h2 className="text-xl font-bold text-white">Model Registry & Evaluation Benchmark</h2><p className="text-xs text-gray-400 mt-1">Computed diagnostics on development evaluation sets, not live audience validation.</p></div>
        ...
        <div key={m.model_name} className="bg-[#0d121f] border border-gray-800 rounded-2xl p-5 shadow-xl flex flex-col justify-between">
          <div>
            <div className="flex flex-col items-start gap-2 mb-3">
              <div><h3 className="font-bold text-sm text-white">{m.model_name.replaceAll("_", " ")}</h3><span className="text-[10px] uppercase text-gray-500 font-semibold">{m.task} • experimental</span></div>
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${m.meets_research_baseline?'bg-emerald-500/10 text-emerald-400 border border-emerald-500/30':'bg-amber-500/10 text-amber-400 border border-amber-500/30'}`}>Synthetic diagnostic</span>
            </div>
            <div className="space-y-1 text-xs text-gray-400 mt-4">
              <div className="flex justify-between"><span>Primary Metric</span><strong className="text-white font-mono">{m.accuracy != null ? `${m.accuracy}% Acc` : m.r2_score != null ? `${m.r2_score} R²` : "Not applicable"}</strong></div>
