/**
 * AISMM Multi-Platform Social OAuth Configuration & Redirect Matrix
 * Supports dynamic local development and production environment domains.
 */

export type SocialPlatform = 'meta' | 'instagram' | 'facebook' | 'twitter' | 'x' | 'linkedin' | 'google' | 'youtube';
...
/**
 * Determine dynamic base URL for OAuth callbacks based on environment.
 */
export function getBaseAppUrl(): string {
  if (typeof window !== 'undefined' && window.location && window.location.origin) {
    return window.location.origin;
  }
  return process.env.VITE_APP_URL || process.env.FRONTEND_URL || 'http://localhost:3000';
}

/**
 * Construct exact authorized callback URI for a specific platform.
 */
export function getRedirectUri(provider: SocialPlatform): string {
...
export const SOCIAL_OAUTH_CONFIG: Record<SocialPlatform, PlatformOAuthConfig> = {
  meta: {
    name: 'Meta',
    provider: 'meta',
