...
class EncryptedText(TypeDecorator):
    """Encrypt at the ORM boundary; callers only see plaintext in memory."""
    impl = Text
    cache_ok = True
    ...
    def process_bind_param(self, value, dialect):
        ...
        return SecretVault().encrypt(value)

    def process_result_value(self, value, dialect):
        if value is None:
            return None
        from backend.app.core.vault import SecretVault
        # Legacy values are encrypted by the upgrade migration.
...
class OAuthAttempt(Base):
    ...
    verifier = Column(EncryptedText(), nullable=True)
    expires_at = Column(DateTime, nullable=False)
...
class User(Base):
    ...
    two_factor_last_step = Column(Integer, nullable=True)
    ...
    phone_verification_token = Column(String(255), nullable=True)
...
class SocialAccount(Base):
    ...
    is_active = Column(Boolean, default=True, nullable=False)
    connected_at = Column(DateTime, default=lambda: datetime.now(UTC).replace(tzinfo=None), nullable=False)
...
class Comment(Base):
    ...
    created_at = Column(DateTime, default=lambda: datetime.now(UTC).replace(tzinfo=None), nullable=False)
    fetched_at = Column(DateTime, default=lambda: datetime.now(UTC).replace(tzinfo=None), nullable=False)
...
class Metric(Base):
    ...
    fetched_at = Column(DateTime, default=lambda: datetime.now(UTC).replace(tzinfo=None), nullable=False, index=True)
...
class StrategyFeedback(Base):
    ...
    id = Column(GUID(), primary_key=True, default=uuid4)
    user_id = Column(GUID(), ForeignKey('users.id', ondelete='CASCADE'), nullable=False, index=True)
    recommendation_id = Column(String(255), nullable=False)
