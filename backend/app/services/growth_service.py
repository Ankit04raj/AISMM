...
class GrowthService:
    ...
    async def get_account_projections(self, account_id: UUID, user_id: UUID) -> AccountGrowthProjectionResponse:
        ...
        current_followers = (account.account_metadata or {}).get("followers_count", 0)
        ...
        post_count = posts_res.scalar() or 0
        ...
        res = self.engine.predict_growth(
            ...
            avg_engagement_rate=float((account.account_metadata or {}).get("engagement_rate", 0)),
        )
        ...
        # Advisory result only; account IDs must never masquerade as ML model foreign keys.
