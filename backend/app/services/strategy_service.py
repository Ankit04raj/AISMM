...
class StrategyService:
    ...
    async def generate_content_plan(
        ...
        # This is a generated advisory plan, not a persisted trained-model prediction.
        return plan
    ...
    async def record_feedback(self, user_id: UUID, request: StrategyFeedbackRequest) -> Dict[str, Any]:
        ...
        self.db.add(StrategyFeedback(user_id=user_id, recommendation_id=request.recommendation_id,
            applied=request.applied, feedback_notes=request.feedback_notes))
        await self.db.commit()
        return {"status":"recorded", "recommendation_id":request.recommendation_id, "applied":request.applied,
                "recorded_at":datetime.now(timezone.utc).isoformat(), "message":"Feedback saved; model retraining is not automatic."}
