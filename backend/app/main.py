...
def create_app() -> FastAPI:
    ...
    async def platform_error_handler(request: Request, exc: PlatformError):
        return JSONResponse(
            ...
            content={"error": "PLATFORM_ERROR", "message": "Platform request failed. Review permissions or reconnect the account.", "platform": exc.platform},
        )
    ...
    async def request_validation_handler(request: Request, exc: RequestValidationError):
        return JSONResponse(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            content={"error": "VALIDATION_ERROR", "message": "Invalid request", "details": [{"loc": list(e["loc"]), "msg": e["msg"], "type": e["type"]} for e in exc.errors()]},
        )
    ...
    # Root - serve landing page for browsers, JSON for API clients
    @app.get("/", tags=["Root"])
    async def root(request: Request):
        import os
        accept = request.headers.get("accept", "")
        # Go up 3 levels: backend/app/main.py -> backend/app -> backend -> repo root
        landing_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "aismm_portal.html")

        # Serve HTML for browsers, JSON for API clients
        if "text/html" in accept and os.path.exists(landing_path):
