...
# Disable email/SMS notifications in test environment to avoid external dependencies
os.environ.setdefault("ENABLE_EMAIL_NOTIFICATIONS", "false")
os.environ.setdefault("ENABLE_PHONE_VERIFICATION", "false")

from backend.app.main import app
from backend.app.api.deps import get_current_user
from backend.app.db.models import User
from backend.app.core.rate_limit import default_rate_limiter
...
def clear_rate_limiter():
    """Clear the global rate limiter between every test to ensure hermetic isolation.

    The default_rate_limiter is a module-level singleton with in-memory state.
    Without clearing, test order affects results (e.g., registration rate limits
    accumulated across test_email_verification.py and test_auth_and_scoping.py).
    """
