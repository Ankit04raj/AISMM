...
def test_auth_oauth_endpoints():
    ...
    assert resp.status_code == 400  # Reject arbitrary callback URL and caller-supplied state.
...
def test_comments_endpoints(auth_headers):
    """Arbitrary provider IDs are not sufficient ownership proof."""
    assert client.get('/api/v1/comments/posts/instagram/post_123', headers=auth_headers).status_code == 404
    assert client.post('/api/v1/comments/instagram/c_101/reply',json={'text':'Thank you'},headers=auth_headers).status_code == 404
    assert client.delete('/api/v1/comments/instagram/c_101',headers=auth_headers).status_code == 404
    assert client.post('/api/v1/comments/instagram/c_101/hide',headers=auth_headers).status_code == 404
