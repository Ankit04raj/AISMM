...
class TestSchedulingService:
    ...
    async def test_recommend_and_auto_schedule(self):
        ...
        with patch("backend.app.services.post_service.PostService.create_post", new_callable=AsyncMock) as mock_create:
            ...
            assert not mock_db.add.called  # PostService owns the sole schedule record.
...
class TestBackgroundScheduledExecutionProof:
    ...
    async def test_automatic_background_execution_without_manual_trigger(self):
        ...
        async with session_factory() as session:
            ...
            session.add(SocialAccount(user_id=user_id, platform='instagram', platform_user_id='scheduler-owner', access_token='test-scheduler-provider-token'))
        ...
        with patch.object(PlatformRegistry, "get_adapter", return_value=adapter), patch.object(adapter, "publish_post", new_callable=AsyncMock, return_value=mock_result):
