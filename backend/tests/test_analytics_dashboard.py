...
class TestAnalyticsService:
    ...
    async def test_content_performance_and_roi(self):
        ...
        assert res.top_performing_hashtags == []  # No fabricated attribution.
    ...
    async def test_temporal_analytics_heatmap(self):
        ...
        assert res.best_overall_hour is None
        assert res.best_overall_day == "insufficient_data"
    ...
    async def test_growth_accuracy_drift_report(self):
        ...
        assert res.r2_score is None
        assert res.drift_status == "insufficient_data"
...
class TestAnalyticsAPIEndpoints:
    ...
    def test_temporal_endpoint(self):
        """HTTP schema test; real empty-database behavior is covered by completion tests."""
        payload={"best_overall_hour":None,"best_overall_day":"insufficient_data", "weekday_avg_engagement":0,
                 "weekend_avg_engagement":0, "weekday_vs_weekend_lift_percent":0, "heatmap_slots":[]}
        with patch("backend.app.services.analytics_service.AnalyticsService.get_temporal_analytics",new_callable=AsyncMock,return_value=payload):
            resp=client.get('/api/v1/analytics/temporal?days=30')
        assert resp.status_code==200
        assert resp.json()['best_overall_hour'] is None
    ...
    def test_growth_accuracy_endpoint(self):
        ...
        assert data["drift_status"] == "insufficient_data"
