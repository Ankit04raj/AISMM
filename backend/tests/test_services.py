...
async def test_post_service_create_and_publish_flow():
    ...
    assert resp.id  # API returns the local post record ID, not the provider ID.
...
async def test_post_service_schedule_flow():
    ...
    user_id = uuid4()
    sched_time = datetime(2027, 1, 1, 12, 0, 0, tzinfo=timezone.utc)
    req = CreatePostRequest(
        platform="mock_service",
        content_type="post",
        text="Scheduled post",
    ...
    assert resp.scheduled_at == sched_time.replace(tzinfo=None)
    assert mock_db.commit.called
