...
class TestSecretConfiguration:
    ...
    def test_missing_secret_key_fails_validation(self, monkeypatch, tmp_path):
        """Settings instantiation must fail when SECRET_KEY or JWT_SECRET_KEY is missing."""
        monkeypatch.chdir(tmp_path)  # Do not read a developer .env during missing-key proof.
    ...
    def test_production_boots_cleanly_with_generated_secure_secrets(self, monkeypatch):
        ...
        monkeypatch.setenv("ENABLE_PHONE_VERIFICATION", "true")
        monkeypatch.setenv("SMS_PROVIDER", "twilio")
        monkeypatch.setenv("FRONTEND_URL", "https://aismm.example.com")
        monkeypatch.setenv("SMTP_USER", "test-user")
        monkeypatch.setenv("SMTP_PASSWORD", "test-provider-password")
...
class TestSecretVaultPerRecordSalt:
    ...
    def test_vault_requires_master_key(self, monkeypatch, tmp_path):
