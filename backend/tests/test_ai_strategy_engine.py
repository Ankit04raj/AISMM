...
class TestStrategyService:
    ...
    async def test_generate_content_plan_service_persists_prediction(self):
        ...
        assert isinstance(plan, ContentStrategyPlan)
        assert not mock_db.add.called  # Advisory plan, not a falsely keyed ML prediction.
...
class TestStrategyAPIEndpoints:
    ...
    def test_record_feedback_endpoint(self):
        ...
        with patch("backend.app.services.strategy_service.StrategyService.record_feedback", new_callable=AsyncMock,
                   return_value={"status":"recorded", "recommendation_id":"rec-test-001", "applied":True}):
