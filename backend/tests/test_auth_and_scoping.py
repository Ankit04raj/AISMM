...
async def async_test_db():
    ...
    options = {"poolclass": NullPool} if TEST_DATABASE_URL.startswith("postgresql") else {}
    engine = create_async_engine(TEST_DATABASE_URL, echo=False, **options)
...
class TestAuthAndUserScoping:
    ...
    async def test_multi_user_data_isolation(self, client, async_test_db):
        ...
        u2_id = UUID(u2_resp.json()["user"]["id"])
        u2_headers = {"Authorization": f"Bearer {u2_token}"}

        # Mark both users as verified so they can access protected endpoints
        from sqlalchemy import update
        await async_test_db.execute(
            update(User).where(User.id == u1_id).values(is_verified=True, phone_verified=True)
        )
        await async_test_db.execute(
            update(User).where(User.id == u2_id).values(is_verified=True, phone_verified=True)
        )
        await async_test_db.commit()
    ...
    @pytest.mark.asyncio
    async def test_email_verification_flow(self, client, async_test_db):
        """Proof 2.1: Real email verification — wrong token rejected, already-verified idempotent."""
        ...
        stmt = select(User).where(User.email == "unverified_user@aismm.io")
